...
describe('streamLinkSummary', () => {
  ...
  it('returns cached summary instantly via SSE on cache hit', async () => {
    ...
    vi.mocked(summaryCache.getCachedSummary).mockResolvedValueOnce(
      'Cached summary text',
    );

    const res = await request(createApp())
      .get(`/links/${linkId}/summary`)
      .buffer(true);
  ...
  it('streams tokens from LLM on cache miss', async () => {
    ...
    vi.mocked(anthropicService.streamSummary).mockImplementationOnce(
      async (_content, _url, callbacks) => {
        ...
        await callbacks.onDone('Hello world', {
          inputTokens: 100,
          outputTokens: 10,
        });
      },
    ...
    expect(summaryCache.cacheSummary).toHaveBeenCalledWith(
      'abc123',
      'Hello world',
    );
    expect(linksRepo.updateLinkSummary).toHaveBeenCalledWith(
      linkId,
      'Hello world',
      'complete',
    );
